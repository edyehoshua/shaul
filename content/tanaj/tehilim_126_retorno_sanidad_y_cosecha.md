---
title: "Tehilim 126: retorno, sanidad y cosecha"
description: "Estudio canónico de Tehilim 126 sobre el retorno de Sion, la alegría pública, la restauración interior y la cosecha perseverante."
date: 2026-07-29
tags:
  - tehilim
  - retorno
  - sion
  - restauracion
references:
  - "#tehilim_126_1-6"
  - "#devarim_30_1-10"
  - "#ieshaiahu_51_11"
  - "#ieshaiahu_57_18-19"
  - "#oseas_14_5-7"
  - "#colosenses_3_10"
sources:
  - "https://www.youtube.com/watch?v=KZsPjtLou_g"
  - "docs/scriptures/oe/json/psalms/126.json"
source_ids:
  - "youtube:KZsPjtLou_g"
translation: "[OE]"
---

# Tesis

Tehilim 126 canta el retorno como una obra que trae alegría, testimonio entre las naciones y una cosecha que nace de lágrimas. La clase de Eric de Jesús Rodríguez Mendoza se detiene en la forma hebrea de “volver” y propone que יהוה no solo ordena el regreso a Sion, sino que acompaña y sana a quienes regresan. El texto confirma el movimiento de restauración y la perseverancia, pero obliga a distinguir entre la forma gramatical, la aplicación pastoral de sanidad y una lectura mesiánica posterior.

## Alcance de la nota

Se sigue el salmo en orden textual. La nota conserva las observaciones materiales de Eric y las contrasta con el texto hebreo local; la transcripción automática no se presenta como cita literal. El hebreo fue normalizado sin barras morfológicas.

## Hoja de comparación

| Referencia       | Texto hebreo local                                                                                                        | Función en el argumento                                                    |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| #tehilim_126_1-2 | **שיר המעלות בשוב יהוה את שיבת ציון היינו כחלמים / אז ימלא שחוק פינו ולשוננו רנה אז יאמרו בגוים הגדיל יהוה לעשות עם אלה** | Retorno de Sion, asombro, boca llena de risa y testimonio de las naciones. |
| #tehilim_126_3-4 | **הגדיל יהוה לעשות עמנו היינו שמחים / שובה יהוה את שבותנו שביתנו כאפיקים בנגב**                                           | Confesión comunitaria y petición de restauración abundante.                |
| #tehilim_126_5-6 | **הזרעים בדמעה ברנה יקצרו / הלוך ילך ובכה נשא משך הזרע בא יבוא ברנה נשא אלמתיו**                                          | Siembra dolorosa, cosecha gozosa y perseverancia del sembrador.            |

## Mapa de la enseñanza de Eric

- **Versículo 1 — el retorno de Sion:** Eric distingue **שיבת** (_shivat_, retorno) de una lectura simplificada de “cautividad”. El texto local contiene **שוב** (_shuv_, volver/regresar) y el sustantivo **שיבה** (_shivah_, retorno); la observación de que el salmo puede poner el acento en volver, y no únicamente en estar cautivo, recibe apoyo textual. La idea de que יהוה vuelve “con” los que regresan es una inferencia pastoral a partir de la preposición y del cuadro completo, no una traducción obligatoria.
- **Versículos 1-2 — “como soñadores” y sanidad:** la clase relaciona **כחלמים** (_kacholמים_, como soñadores) con personas que pasan de un estado de enfermedad o delirio a la alegría restaurada. El hebreo afirma asombro y una alegría repentina; “sanidad” es una aplicación aproximada, no el sentido exclusivo de “soñadores”.
- **Versículos 2-3 — la alegría que proclama:** Eric observa que la boca llena de risa y la lengua de canto no son una emoción privada, sino una confesión visible que hace que los pueblos digan: “Grande ha hecho יהוה”. El texto apoya directamente tanto la voz de la comunidad como el reconocimiento de las naciones.
- **Versículo 4 — restauración como torrentes:** la clase lee **כאפיקים בנגב** (_ka'afikim banegev_, como corrientes en el Néguev) como imagen de una restauración que trae vida a un lugar seco. El símil sí contrasta sequedad y flujo abundante; aplicarlo a la sanidad interior es pedagógico y debe conservar la referencia geográfica del salmo.
- **Versículos 5-6 — lágrimas y cosecha:** Eric insiste en que el sembrador no abandona el campo por llorar, y que el retorno verdadero debe producir obediencia y fruto. El paralelismo “sembrar/cosechar” y “llorar/cantar” sostiene perseverancia y resultado; no promete que cada sufrimiento individual produzca prosperidad inmediata.

## El retorno no es solo traslado

La raíz **שוב** aparece como acción de volver y como sustantivo de retorno. Eric propone escuchar en ella también la teshuvá, el regreso de la persona desde la esclavitud del pecado. Esa conexión es lingüísticamente cercana en la raíz, pero no permite afirmar que cada aparición de **שוב** signifique arrepentimiento completo. Devarim 30:1-10 sí ofrece el puente canónico: retorno geográfico, obediencia y circuncisión del corazón aparecen juntos. Por eso la clase acierta al resistir un regreso meramente externo; el salmo mismo pasa del retorno de Sion a boca, lengua, alegría y siembra.

El versículo 1 dice que “fuimos como soñadores”. Eric conserva dos posibilidades: el estupor de quien no puede creer la restauración y la imagen de quien es levantado de una condición enferma. La primera está directamente en el lenguaje poético; la segunda armoniza con la dirección restauradora de los versículos 4-6, pero es una lectura aproximada. Ieshaiahu 51:11 confirma el patrón de regreso con alegría, mientras que Oseas 14:5-7 permite conectar sanidad con retorno sin convertir ambas profecías en una glosa del salmo.

## Alegría que vuelve visible la obra de יהוה

En los versículos 2-3 el sujeto de **הגדיל** (_higdil_, ha engrandecido/hizo grande) es יהוה. Eric subraya que la sonrisa no debe ser una alegría sin motivo, sino el efecto de proclamar sus maravillas. El texto prueba ese énfasis: la risa de la boca, el canto de la lengua y la frase de los pueblos convergen en una obra reconocible de יהוה. La conexión con Colosenses 3:10 —renovación conforme a la imagen del creador— es canónica y pedagógica; no debe sustituir el sentido histórico del retorno.

El salmo permite hablar de alegría corporal y pública sin negar la formación interior. La persona vuelve, pero también aprende a sembrar. Por eso la aplicación de Eric contra una profesión religiosa separada de la obediencia es coherente con el movimiento del poema, aunque la palabra “sanidad” sea más amplia que el vocabulario explícito de 126.

## Léxico clave

| Forma      | Transliteración | Sentido normal                             | Fuerza contextual y relación                                                                              |
| ---------- | --------------- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| **(שוב)**  | _shuv_          | volver, regresar, restaurar según contexto | En 126:1 y 4 organiza el retorno; relación con teshuvá aproximada y canónica, no identidad automática.    |
| **(שיבה)** | _shivah_        | retorno, vuelta                            | Nombra el regreso de Sion; no significa por sí sola “cautividad”.                                         |
| **(חלם)**  | _chalam_        | soñar                                      | En **כחלמים**, “como soñadores”; sanidad es aplicación pedagógica.                                        |
| **(שחוק)** | _sechoq_        | risa                                       | La boca llena de risa expresa alegría restaurada; no es frivolidad aislada.                               |
| **(רנה)**  | _rinah_         | grito/canto gozoso                         | La lengua proclama públicamente; “alabanza” es aproximación contextual.                                   |
| **(אפיק)** | _afik_          | cauce, corriente                           | La imagen de corrientes en el Néguev comunica abundancia en la sequedad; la sanidad interior es analogía. |
| **(דמע)**  | _dama_          | lágrima                                    | Las lágrimas acompañan la siembra; no son evidencia de fracaso.                                           |

## Conexiones principales

- #devarim_30_1-10 relaciona regreso, obediencia y restauración del corazón.
- #ieshaiahu_51_11 confirma el retorno de Sion acompañado de gozo.
- #oseas_14_5-7 desarrolla el lenguaje de sanar la apostasía y florecer.
- #colosenses_3_10 ofrece una conexión mesiánica sobre renovación, marcada como aproximada.

## Pendiente de verificar

- [ ] Precisar la tradición comentarial que Eric resume sobre “como soñadores” y su posible sentido de sanidad.
- [ ] Revisar en una edición crítica la discusión sintáctica de **בשוב** y el valor exacto de la preposición en la primera línea.

## Conclusión

Tehilim 126 no separa retorno, alegría y fruto. יהוה restaura a Sion, la comunidad reconoce la grandeza de la obra, y los que siembran llorando reciben una promesa de cosecha gozosa. Eric ayuda a ver que regresar a la tierra o a la comunidad no basta si el corazón no vuelve; el texto añade que la alegría restaurada debe hacerse canto y que la esperanza persevera aun cuando la semilla todavía está oculta.

## Créditos

- Expositor: **hermano Eric de Jesús Rodríguez Mendoza**.
- Video: [Tehilim/Hymnos/Salmo Chap 126](https://www.youtube.com/watch?v=KZsPjtLou_g) (`source_id`: `youtube:KZsPjtLou_g`).
- Esta nota organiza y contrasta la exposición; no presenta la transcripción automática como cita literal.
